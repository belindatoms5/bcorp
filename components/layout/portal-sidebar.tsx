...
import { Building2, LayoutDashboard, Receipt, Users, Bell, LogOut, ChevronDown } from "lucide-react";
...
export function PortalSidebar({ schemeId, schemeName, userName }: PortalSidebarProps) {
  const pathname = usePathname();
  const router = useRouter();
  const supabase = createClient();

  const nav = [
    { label: "My overview", icon: LayoutDashboard, href: `/portal/${schemeId}` },
    { label: "My levies", icon: Receipt, href: `/portal/${schemeId}/levies` },
    { label: "Meetings", icon: Users, href: `/portal/${schemeId}/meetings` },
    { label: "Announcements", icon: Bell, href: `/portal/${schemeId}/announcements` },
  ];
